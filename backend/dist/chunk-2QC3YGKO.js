  ...
  async uploadFile(options2) {
    ...
    if (domain) {
      ...
      if (process.env.NODE_ENV === "production") {
        log5.warn("[GoogleDrive] WARNING: GOOGLE_WORKSPACE_DOMAIN not set. Files will be shared with anyone who has the link. Set this env var for domain-restricted access.");
      }
      await drive.permissions.create({
        fileId: response.data.id,
        requestBody: {
