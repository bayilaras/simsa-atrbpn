import {
  arsipService
} from "./chunk-PXURFYUM.js";
import {
  arsipVitalService
} from "./chunk-VFKTQ3F4.js";
import {
  arsipTerjagaService
} from "./chunk-BZ77VRLB.js";
import {
  db,
  env
...
var auth = betterAuth({
  ...
  advanced: {
    ...
    // We have our own CSRF middleware; Better Auth's check conflicts with Vercel proxy
    defaultCookieAttributes: env.NODE_ENV === "production" ? {
      sameSite: "none",
      // Required for cross-domain cookies on .vercel.app (public suffix)
      secure: true,
      partitioned: true
      // Required by modern browsers for third-party cookies
    } : {},
    crossSubDomainCookies: env.COOKIE_DOMAIN ? { enabled: true, domain: env.COOKIE_DOMAIN } : void 0,
    database: {
      generateId: "uuid"
...
var rateLimit = rateLimitModule.default || rateLimitModule;
...
import { eq as eq2, and, desc, sql, gte, lte, or, ilike, isNull } from "drizzle-orm";
var SuratMasukService = class {
  async findAll(filters) {
    ...
    const conditions = [
      ...
      or(eq2(suratMasuk.isDeleted, false), isNull(suratMasuk.isDeleted))
      // Exclude soft-deleted records (NULL-safe)
    ];
    if (tahun) {
  ...
  async getStats(unitKerjaId, tahun) {
    try {
      const baseConditions = [
        or(eq2(suratMasuk.isDeleted, false), isNull(suratMasuk.isDeleted)),
        ...unitKerjaId ? [eq2(suratMasuk.unitKerjaId, unitKerjaId)] : [],
        ...tahun ? [eq2(suratMasuk.tahun, tahun)] : []
      ];
      const [
        [totalResult],
        [belumDibalasResult],
        ...
        db.select({ count: sql`count(*)::int` }).from(suratMasuk).where(and(...baseConditions)),
        db.select({ count: sql`count(*)::int` }).from(suratMasuk).where(and(...baseConditions, eq2(suratMasuk.status, "belum_dibalas"))),
        db.select({ count: sql`count(*)::int` }).from(suratMasuk).where(and(...baseConditions, eq2(suratMasuk.status, "sudah_dibalas"))),
        db.select({ count: sql`count(*)::int` }).from(suratMasuk).where(and(...baseConditions, eq2(suratMasuk.isArchived, true)))
      ]);
      const result = {
  ...
  async getBalasan(suratMasukId) {
    const { suratKeluar: suratKeluar3 } = await import("./schema-AGPFRNYX.js");
  ...
  async findByIdWithLinks(id) {
    ...
    const { arsip: arsip3 } = await import("./schema-AGPFRNYX.js");
...
var FULL_ADMIN = ["super_admin", "admin_dirjen", "admin_sesditjen"];
var PERMISSIONS = {
  surat_masuk: {
  ...
  arsip: {
    ...
    delete: FULL_ADMIN,
    destroy: ["super_admin"],
    // Only super_admin can permanently destroy archives
    export: [...FULL_ADMIN, "auditor"]
  },
  arsip_vital: {
    read: [...FULL_ADMIN, "staff", "auditor"],
...
var log5 = createLogger("BlobStorageService");
var BlobStorageService = class {
  // Upload file to Vercel Blob
  async uploadFile(options2) {
    const { fileName, mimeType, buffer, folder } = options2;
    const pathname = folder ? `${folder}/${fileName}` : `uploads/${fileName}`;
    log5.info({ fileName, mimeType, bufferSize: buffer.length, pathname }, "Uploading file to Vercel Blob");
    ...
    log5.info({ url: blob.url, pathname: blob.pathname }, "File uploaded to Vercel Blob");
    return {
      id: blob.url,
  ...
  async getFile(blobUrl) {
    try {
      ...
      log5.error({ err: error, blobUrl }, "Failed to get file metadata");
      return null;
    }
  }
  ...
  async deleteFile(blobUrl) {
    try {
      await del(blobUrl);
      log5.info({ blobUrl }, "File deleted from Vercel Blob");
      return true;
    } catch (error) {
      log5.error({ err: error, blobUrl }, "Failed to delete file");
      return false;
    }
  }
  ...
  async downloadFile(blobUrl) {
    try {
      ...
      log5.error({ err: error, blobUrl }, "Failed to download file from Blob");
      return null;
    }
  }
  // List files
  async listFiles(prefix) {
    ...
    return result.blobs.map((blob) => ({
      ...
      // list doesn't return content type
      url: blob.url,
      downloadUrl: blob.downloadUrl,
      size: blob.size
    }));
...
// src/utils/resolve-unit-kerja.ts
function resolveUnitKerjaId(req) {
  const role = req.user?.role || "user";
  const userUnitKerjaId = req.user?.unitKerjaId || null;
  const queryUnitKerjaId = req.query?.unitKerjaId || null;
  switch (role) {
    case "super_admin":
...
// src/routes/surat-masuk.routes.ts
var log6 = createLogger("SuratMasukRoutes");
var upload = multer({
...
router.get("/stats", async (req, res, next) => {
  try {
    ...
    log6.info({ stats }, "[GET /stats] Stats result");
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      let fileOriginalName = req.body.fileOriginalName || null;
      if (file && file.buffer && !filePath) {
        try {
          ...
          log6.info({ blobUrl: blobFile.url, fileName: file.originalname }, "File uploaded to Vercel Blob");
        } catch (uploadError) {
          log6.error({ err: uploadError }, "Failed to upload file to Vercel Blob");
...
router.put(
  ...
  async (req, res, next) => {
    try {
      const id = req.params.id;
      const file = req.file;
      log6.info({ id, hasFile: !!file }, "[PUT /surat-masuk/:id] Request received");
      log6.info({ bodyKeys: Object.keys(req.body) }, "[PUT /surat-masuk/:id] Body keys");
      const existing = await suratMasukService.findById(id);
      if (!existing) {
        return res.status(404).json({ error: "Surat masuk not found" });
      }
      const bodyValidation = updateSuratMasukSchema.safeParse(req.body);
      log6.info({ valid: bodyValidation.success }, "[PUT /surat-masuk/:id] Validation result");
      if (!bodyValidation.success) {
        log6.info({ errors: bodyValidation.error.issues }, "[PUT /surat-masuk/:id] Validation errors");
      }
      let updateData;
      if (bodyValidation.success) {
      ...
      if (file && file.buffer && !updateData.filePath) {
        try {
          ...
          log6.info({ blobUrl: blobFile.url, fileName: file.originalname }, "File uploaded to Vercel Blob (update)");
        } catch (uploadError) {
          log6.error({ err: uploadError }, "Failed to upload file to Vercel Blob");
          return res.status(500).json({ success: false, error: "Gagal mengunggah file", message: uploadError?.message || "Unknown error" });
        }
      }
      log6.info({ updateKeys: Object.keys(updateData) }, "[PUT /surat-masuk/:id] Update data keys");
      ...
      log6.error({ err: error, message: error?.message, stack: error?.stack }, "[PUT /surat-masuk/:id] Error:");
      res.status(500).json({
        success: false,
...
router.post("/:id/archive-full", canWriteMiddleware(), async (req, res, next) => {
  try {
    const id = req.params.id;
    const { arsipService: arsipService2 } = await import("./arsip.service-L7QNQCGG.js");
...
var SuratKeluarService = class {
  ...
  async getStats(unitKerjaId, tahun) {
    const conditions = [
      ...unitKerjaId ? [eq4(suratKeluar.unitKerjaId, unitKerjaId)] : [],
      ...tahun ? [eq4(suratKeluar.tahun, tahun)] : []
    ];
    const whereClause = conditions.length > 0 ? and3(...conditions) : void 0;
    const stats = await db.select({
      total: sql3`count(*)::int`,
      diarsipkan: sql3`sum(case when ${suratKeluar.isArchived} = true then 1 else 0 end)::int`
    }).from(suratKeluar).where(whereClause);
...
// src/routes/surat-keluar.routes.ts
var log7 = createLogger("SuratKeluarRoutes");
var upload2 = multer2({
    ...
    try {
      ...
      if (file && file.buffer && !filePath) {
        try {
          ...
          filePath = `blob:${blobFile.url}`;
          fileOriginalName = file.originalname;
          log7.info({ blobUrl: blobFile.url, fileName: file.originalname }, "File uploaded to Vercel Blob");
        } catch (uploadError) {
          log7.error({ err: uploadError }, "Failed to upload file to Vercel Blob");
          return res.status(500).json({ success: false, error: "Gagal mengunggah file", message: uploadError?.message || "Unknown error" });
        }
      }
    ...
    try {
      ...
      if (file && file.buffer && !updateData.filePath) {
        try {
          ...
          updateData.filePath = `blob:${blobFile.url}`;
          updateData.fileOriginalName = file.originalname;
          log7.info({ blobUrl: blobFile.url, fileName: file.originalname }, "File uploaded to Vercel Blob (update)");
        } catch (uploadError) {
          log7.error({ err: uploadError }, "Failed to upload file to Vercel Blob");
          return res.status(500).json({ success: false, error: "Gagal mengunggah file", message: uploadError?.message || "Unknown error" });
        }
      ...
      log7.error({ err: error, message: error?.message, stack: error?.stack }, "[PUT /surat-keluar/:id] Error:");
      res.status(500).json({
        success: false,
...
// src/routes/upload.routes.ts
var log8 = createLogger("UploadRoutes");
var router4 = Router4();
...
router4.post(
  ...
  async (req, res) => {
    try {
      ...
      log8.error({ err: error }, "Upload error:");
...
router4.get("/:suratType/:suratId", authMiddleware, async (req, res) => {
  try {
    ...
    log8.error({ err: error }, "Get attachments error:");
...
router4.delete("/:id", authMiddleware, async (req, res) => {
  try {
    ...
    log8.error({ err: error }, "Delete attachment error:");
...
var log9 = createLogger("DashboardService");
var dashboardService = {
  ...
  async getUnitKerjaComparison(unitKerjaId, tahun) {
    try {
      ...
      log9.error({ err: error }, "[DashboardService] Error in getUnitKerjaComparison:");
      throw error;
    }
  }
};
...
// src/routes/export.routes.ts
var log10 = createLogger("ExportRoutes");
var router8 = Router8();
...
router8.get("/surat-masuk/excel", async (req, res) => {
  try {
    ...
    log10.error({ err: error }, "Error exporting Surat Masuk to Excel:");
...
router8.get("/surat-masuk/pdf", async (req, res) => {
  try {
    ...
    log10.error({ err: error }, "Error exporting Surat Masuk to PDF:");
...
router8.get("/surat-keluar/excel", async (req, res) => {
  try {
    ...
    log10.error({ err: error }, "Error exporting Surat Keluar to Excel:");
    res.status(500).json({ error: "Failed to export to Excel" });
  }
});
router8.get("/surat-keluar/pdf", async (req, res) => {
  try {
    ...
    log10.error({ err: error }, "Error exporting Surat Keluar to PDF:");
    res.status(500).json({ error: "Failed to export to PDF" });
  }
});
router8.get("/arsip/excel", async (req, res) => {
  try {
    ...
    log10.error({ err: error }, "Error exporting Arsip to Excel:");
    res.status(500).json({ error: "Failed to export to Excel" });
  }
});
router8.get("/arsip/pdf", async (req, res) => {
  try {
    ...
    log10.error({ err: error }, "Error exporting Arsip to PDF:");
    res.status(500).json({ error: "Failed to export to PDF" });
  }
});
...
// src/routes/notification.routes.ts
var log11 = createLogger("NotificationRoutes");
var router9 = Router9();
...
router9.get("/", async (req, res) => {
  try {
    ...
    log11.error({ err: error }, "Error fetching notifications:");
...
router9.get("/count", async (req, res) => {
  try {
    ...
    log11.error({ err: error }, "Error fetching notification count:");
...
router9.get("/surat-masuk", async (req, res) => {
  try {
    ...
    log11.error({ err: error }, "Error fetching surat masuk notifications:");
    res.status(500).json({ error: "Failed to fetch notifications" });
  }
});
router9.get("/arsip", async (req, res) => {
  try {
    ...
    log11.error({ err: error }, "Error fetching arsip notifications:");
    res.status(500).json({ error: "Failed to fetch notifications" });
  }
});
router9.patch("/:id/read", async (req, res) => {
  try {
    ...
    log11.error({ err: error }, "Error marking notification as read:");
...
router9.patch("/read-all", validateBody(markAllReadSchema), async (req, res) => {
  try {
    ...
    log11.error({ err: error }, "Error marking all notifications as read:");
...
var VALID_ROLES = ["super_admin", "admin_dirjen", "admin_sesditjen", "staff", "user"];
...
var userManagementService = {
  ...
  async createUser(data) {
    ...
    const [existingUser] = await db.select({ id: users.id }).from(users).where(eq9(users.email, data.email)).limit(1);
    if (existingUser) {
...
var roles = ["super_admin", "admin_dirjen", "admin_sesditjen", "staff", "user"];
...
var createUserSchema = z2.object({
  ...
  name: z2.string().min(1, "Nama wajib diisi").max(255),
  role: z2.enum(roles),
  unitKerjaId: z2.string().nullable().optional(),
  jabatan: z2.string().max(100).nullable().optional(),
  nip: z2.string().max(30).nullable().optional(),
  password: z2.string().min(8, "Password minimal 8 karakter").optional()
});

// src/routes/user-management.routes.ts
var log12 = createLogger("UserManagementRoutes");
...
router10.get("/", requireAdmin, async (req, res) => {
  try {
    ...
    log12.error({ err: error }, "List users error:");
    res.status(500).json({ error: "Internal server error" });
  }
});
router10.post("/", requireAdmin, async (req, res) => {
  try {
    ...
    const newUser = await user_management_service_default.createUser(bodyResult.data);
    ...
    log12.error({ err: error }, "Create user error:");
...
router10.get("/roles", requireAdmin, async (req, res) => {
  try {
    ...
    log12.error({ err: error }, "Get roles error:");
    res.status(500).json({ error: "Internal server error" });
  }
});
router10.get("/unit-kerja", requireAdmin, async (req, res) => {
  try {
    ...
    log12.error({ err: error }, "Get unit kerja error:");
    res.status(500).json({ error: "Internal server error" });
  }
});
router10.get("/:userId", requireAdmin, async (req, res) => {
  try {
    ...
    log12.error({ err: error }, "Get user error:");
    res.status(500).json({ error: "Internal server error" });
  }
});
router10.put("/:userId", requireAdmin, async (req, res) => {
  try {
    ...
    log12.error({ err: error }, "Update user error:");
...
router10.delete("/:userId", requireAdmin, async (req, res) => {
  try {
    ...
    log12.error({ err: error }, "Deactivate user error:");
    res.status(500).json({ error: "Internal server error" });
  }
});
...
var log13 = createLogger("AuditLogRoutes");
...
async function requireAuth(req, res, next) {
  try {
    ...
    log13.error({ err: error }, "Auth check error:");
    res.status(500).json({ error: "Internal server error" });
  }
}
router11.get("/", requireAuth, async (req, res) => {
  try {
    ...
    log13.error({ err: error }, "List audit logs error:");
    res.status(500).json({ error: "Internal server error" });
  }
});
router11.get("/:entityType/:entityId", requireAuth, async (req, res) => {
  try {
    ...
    log13.error({ err: error }, "Get entity history error:");
    res.status(500).json({ error: "Internal server error" });
  }
});
...
// src/routes/klasifikasi.routes.ts
var log14 = createLogger("KlasifikasiRoutes");
...
router12.get("/", async (req, res) => {
  try {
    ...
    log14.error({ err: error }, "Error fetching klasifikasi:");
    res.status(500).json({ error: "Internal server error" });
  }
});
router12.get("/stats", async (req, res) => {
  try {
    ...
    log14.error({ err: error }, "Error fetching stats:");
    res.status(500).json({ error: "Internal server error" });
  }
});
...
router12.post("/", async (req, res) => {
  try {
    ...
    log14.error({ err: error }, "Error creating klasifikasi:");
    res.status(500).json({ error: "Internal server error" });
  }
});
router12.put("/:kode", async (req, res) => {
  try {
    ...
    log14.error({ err: error }, "Error updating klasifikasi:");
    res.status(500).json({ error: "Internal server error" });
  }
});
router12.delete("/:kode", async (req, res) => {
  try {
    ...
    log14.error({ err: error }, "Error deleting klasifikasi:");
    res.status(500).json({ error: "Internal server error" });
  }
});
...
var log15 = createLogger("JraRoutes");
...
router13.post("/", async (req, res) => {
  try {
    ...
    log15.error({ err: error }, "Error creating JRA:");
    res.status(500).json({ error: "Internal server error" });
  }
});
router13.put("/:kode", async (req, res) => {
  try {
    ...
    log15.error({ err: error }, "Error updating JRA:");
    res.status(500).json({ error: "Internal server error" });
  }
});
router13.delete("/:kode", async (req, res) => {
  try {
    ...
    log15.error({ err: error }, "Error deleting JRA:");
    res.status(500).json({ error: "Internal server error" });
  }
});
...
var log16 = createLogger("ArsipPickerRoutes");
...
router14.get("/klasifikasi/tree", authMiddleware, async (req, res) => {
  try {
    ...
    log16.error({ err: error }, "Error fetching klasifikasi tree:");
...
router14.get("/jra/:kode", authMiddleware, async (req, res) => {
  try {
    ...
    log16.error({ err: error }, "Error fetching JRA:");
...
router14.post("/calculate-dates", authMiddleware, async (req, res) => {
  try {
    ...
    log16.error({ err: error }, "Error calculating dates:");
...
router14.get("/lifecycle", authMiddleware, async (req, res) => {
  try {
    ...
    log16.error({ err: error }, "Error fetching lifecycle notifications:");
...
import { eq as eq12, and as and10, sql as sql7, isNull as isNull3 } from "drizzle-orm";
...
var StorageLocationService = class {
  async findAll(filters) {
    ...
    if (parentId === null) {
      conditions.push(isNull3(storageLocations.parentId));
    } else if (parentId) {
...
// src/routes/dosir.routes.ts
var log17 = createLogger("DosirRoutes");
...
router17.get("/", async (req, res) => {
  try {
    ...
    log17.error({ err: error }, "Error fetching dosir:");
...
router17.get("/stats", async (req, res) => {
  try {
    ...
    log17.error({ err: error }, "Error fetching stats:");
...
router17.get("/generate-kode", async (req, res) => {
  try {
    ...
    log17.error({ err: error }, "Error generating kode:");
...
router17.get("/:id/timeline", async (req, res) => {
  try {
    ...
    log17.error({ err: error }, "Error fetching timeline:");
...
router17.post("/", validateBody(createDosirSchema), async (req, res) => {
  try {
    ...
    log17.error({ err: error }, "Error creating dosir:");
...
router17.put("/:id", validateBody(updateDosirSchema), async (req, res) => {
  try {
    ...
    log17.error({ err: error }, "Error updating dosir:");
...
router17.delete("/:id", sensitiveLimiter, async (req, res) => {
  try {
    ...
    log17.error({ err: error }, "Error deleting dosir:");
...
router17.post("/:id/surat", validateBody(linkSuratToDosirSchema), async (req, res) => {
  try {
    ...
    log17.error({ err: error }, "Error linking surat:");
...
router17.delete("/:id/surat/:type/:suratId", async (req, res) => {
  try {
    ...
    log17.error({ err: error }, "Error unlinking surat:");
...
var log18 = createLogger("RetentionRoutes");
...
router18.get("/summary", async (req, res) => {
  try {
    ...
    log18.error({ err: error }, "Error fetching retention summary:");
...
router18.get("/candidates", async (req, res) => {
  try {
    ...
    log18.error({ err: error }, "Error fetching disposal candidates:");
...
router18.post("/disposal-report", async (req, res) => {
  try {
    ...
    log18.error({ err: error }, "Error generating disposal report:");
...
router18.get("/lifecycle", async (req, res) => {
  try {
    ...
    log18.error({ err: error }, "Error fetching lifecycle notifications:");
...
var log19 = createLogger("OcrService");
var pdfjs = null;
async function loadPdfJs() {
  if (!pdfjs) {
    try {
      ...
      log19.warn("Failed to load pdfjs-dist legacy build, PDF extraction disabled");
      pdfjs = null;
    }
  }
  return pdfjs;
...
var OCRService = class {
  ...
  async extractTextFromPDF(buffer) {
    try {
      ...
      log19.error({ err: error }, "Error extracting text from PDF:");
      throw error;
    }
  }
  ...
  async performOCR(imageBuffer) {
    try {
      const { data: { text } } = await Tesseract.recognize(
        ...
        // Indonesian + English
        {
          logger: (m) => log19.info(`OCR Progress: ${m.status} - ${Math.round((m.progress || 0) * 100)}%`)
        }
      );
      return text;
    } catch (error) {
      log19.error({ err: error }, "OCR Error:");
      throw error;
    }
  }
  ...
  async processPDF(buffer) {
    try {
      ...
      if (extractedText.length < 50) {
        log19.info("PDF text extraction yielded little text, attempting OCR...");
      ...
      log19.error({ err: error }, "PDF processing error:");
      return {
        success: false,
        text: "",
        metadata: {
...
var log20 = createLogger("BulkUploadService");
  ...
  async processBatch(batchId, files, folderId) {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        log20.error(`Error processing file ${file.fileName}:`, error);
        item.status = "failed";
  ...
  async confirmBatch(batchId, confirmedItems, files, folderId) {
    ...
    for (const confirmedItem of confirmedItems) {
      ...
      try {
        ...
        log20.error(`Error saving arsip for ${batchItem.fileName}:`, error);
...
// src/routes/bulk-upload.routes.ts
var log21 = createLogger("BulkUploadRoutes");
  ...
  async (req, res) => {
    try {
      ...
      bulkUploadService.processBatch(batch.batchId, uploadFiles, folderId).catch((error) => {
        log21.error({ err: error }, "Batch processing error:");
      });
      res.json({
        success: true,
        data: {
      ...
      log21.error({ err: error }, "Bulk upload error:");
      res.status(500).json({
        success: false,
...
router19.get("/:batchId", authMiddleware, async (req, res) => {
  try {
    ...
    log21.error({ err: error }, "Get batch error:");
    res.status(500).json({
      success: false,
...
router19.post("/:batchId/confirm", authMiddleware, async (req, res) => {
  try {
    ...
    log21.error({ err: error }, "Confirm batch error:");
    res.status(500).json({
      success: false,
...
// src/routes/report.routes.ts
var log22 = createLogger("ReportRoutes");
...
router21.get("/surat-masuk", async (req, res) => {
  try {
    ...
    log22.error({ err: error }, "Error getting surat masuk report:");
...
router21.get("/surat-keluar", async (req, res) => {
  try {
    ...
    log22.error({ err: error }, "Error getting surat keluar report:");
    res.status(500).json({ error: "Failed to generate report" });
  }
});
router21.get("/arsip", async (req, res) => {
  try {
    ...
    log22.error({ err: error }, "Error getting arsip report:");
    res.status(500).json({ error: "Failed to generate report" });
  }
});
router21.get("/lending", async (req, res) => {
  try {
    ...
    log22.error({ err: error }, "Error getting lending report:");
    res.status(500).json({ error: "Failed to generate report" });
  }
});
router21.get("/summary", async (req, res) => {
  try {
    ...
    log22.error({ err: error }, "Error getting summary report:");
    res.status(500).json({ error: "Failed to generate report" });
  }
});
router21.get("/export/:type/:format", async (req, res) => {
  try {
    ...
    log22.error({ err: error }, "Error exporting report:");
...
// src/routes/settings.routes.ts
var log23 = createLogger("SettingsRoutes");
...
router22.get("/profile", async (req, res) => {
  try {
    ...
    log23.error({ err: error }, "Error getting profile:");
...
router22.put("/profile", async (req, res) => {
  try {
    ...
    log23.error({ err: error }, "Error updating profile:");
...
router22.get("/unit-kerja", async (req, res) => {
  try {
    ...
    log23.error({ err: error }, "Error getting unit kerja:");
...
router22.put("/unit-kerja/:id", async (req, res) => {
  try {
    ...
    log23.error({ err: error }, "Error updating unit kerja:");
...
router22.post("/unit-kerja", async (req, res) => {
  try {
    ...
    log23.error({ err: error }, "Error creating unit kerja:");
...
router22.get("/surat-templates", async (req, res) => {
  try {
    ...
    log23.error({ err: error }, "Error getting surat templates:");
...
router22.put("/surat-templates", async (req, res) => {
  try {
    ...
    log23.error({ err: error }, "Error updating surat templates:");
...
var log24 = createLogger("DevAuthRoutes");
...
router24.post("/dev-login", async (req, res) => {
  ...
  try {
    ...
    if (!session) {
      ...
      const { sessions } = await import("./schema-AGPFRNYX.js");
    ...
    log24.error({ err: error }, "Dev login error:");
    res.status(500).json({ error: "Internal server error" });
  }
});
...
var PrintTemplateService = class {
  ...
  async generateDaftarArsipVital(unitKerjaId) {
    const { data } = await import("./arsip-vital.service-COXDPZHT.js").then((m) => m.arsipVitalService.findAll({ unitKerjaId, limit: 1e3 }));
    const doc = new PDFDocument2({ size: "A4", layout: "landscape", margin: this.MARGIN.left });
    const buffers = [];
    doc.on("data", (chunk) => buffers.push(chunk));
  ...
  async generateDaftarArsipTerjaga(unitKerjaId) {
    const { data } = await import("./arsip-terjaga.service-2FQPMCVI.js").then((m) => m.arsipTerjagaService.findAll({ unitKerjaId, limit: 1e3 }));
    const doc = new PDFDocument2({ size: "A4", layout: "landscape", margin: this.MARGIN.left });
    const buffers = [];
    doc.on("data", (chunk) => buffers.push(chunk));
...
var ArsipElektronikService = class {
  ...
  async addPreservationAction(data) {
    const { preservasiTrack } = await import("./preservasi-track-HVAJ3BHR.js");
  ...
  async getPreservationHistory(arsipElektronikId) {
    const { preservasiTrack, users: users3 } = await import("./schema-AGPFRNYX.js");
    const results = await db.select({
...
var storage = multer6.diskStorage({
...
var log25 = createLogger("HashVerificationService");
var HashVerificationService = class {
  ...
  static async verifyUploadedFile(filePath) {
    try {
      ...
      await unlink(filePath).catch((err) => log25.error({ err }, "Failed to clean up temp file"));
...
var SupervisionService = class {
  ...
  async getComplianceStats() {
    ...
    const { arsipElektronik: arsipElektronik2 } = await import("./arsip-elektronik-R3BEUAAV.js");
...
// src/routes/supervision.routes.ts
var log26 = createLogger("SupervisionRoutes");
...
router32.get("/stats/activity", async (req, res) => {
  try {
    ...
    log26.error({ err: error }, "Error fetching activity stats:");
...
router32.get("/stats/users", async (req, res) => {
  try {
    ...
    log26.error({ err: error }, "Error fetching user stats:");
...
router32.get("/stats/compliance", async (req, res) => {
  try {
    ...
    log26.error({ err: error }, "Error fetching compliance stats:");
...
var log27 = createLogger("MappingRoutes");
var router33 = Router33();
log27.info("\u2705 Mapping routes file loaded");
...
router33.get("/klasifikasi-jra", async (req, res) => {
  try {
    ...
    log27.error({ err: error }, "Error fetching mappings:");
    res.status(500).json({ error: "Internal server error" });
  }
});
router33.get("/suggest-jra/:klasifikasiKode", async (req, res) => {
  try {
    ...
    log27.error({ err: error }, "Error fetching JRA suggestions:");
    res.status(500).json({ error: "Internal server error" });
  }
});
...
var log28 = createLogger("EmailService");
var EmailService = class {
  ...
  constructor() {
    if (process.env.SMTP_HOST && process.env.SMTP_USER) {
      ...
      log28.info("SMTP not configured, using stub mode");
    }
  }
  async sendEmail(options2) {
    if (!this.transporter) {
      log28.debug({ to: options2.to, subject: options2.subject }, "Email stub: message not sent (no SMTP)");
      return true;
    }
    try {
      ...
      log28.error({ err: error }, "Failed to send email");
      return false;
    }
  }
...
// src/services/approval.service.ts
var log29 = createLogger("ApprovalService");
var ApprovalService = class {
  ...
  async submit(suratId, requesterId, nextApproverId, notes) {
    return await db.transaction(async (tx) => {
      ...
      this.sendNotification(suratId, nextApproverId, requesterId).catch((err) => log29.error({ err }, "Failed to send submit notification"));
      return request;
    });
  }
  // Approve step
  async approve(suratId, approverId, notes, nextApproverId) {
    return await db.transaction(async (tx) => {
      ...
      if (nextApproverId) {
        ...
        this.sendNotification(suratId, nextApproverId, request.requesterId).catch((err) => log29.error({ err }, "Failed to send approval notification"));
      } else {
  ...
  async sendNotification(suratId, targetUserId, requesterId) {
    try {
      ...
      log29.error({ err }, "Failed to send notification");
    }
  }
};
...
var log30 = createLogger("GoogleDriveImportService");
...
// src/routes/google-drive-import.routes.ts
var log31 = createLogger("GoogleDriveImportRoutes");
...
router36.get("/google-drive/sheets", async (req, res) => {
  try {
    ...
    log31.error({ err: error }, "Error listing sheets:");
...
router36.post("/google-drive/preview", async (req, res) => {
  try {
    ...
    log31.error({ err: error }, "Error previewing data:");
...
router36.post("/google-drive/surat-masuk", async (req, res) => {
  try {
    ...
    log31.error({ err: error }, "Error importing surat masuk:");
...
router36.post("/google-drive/surat-keluar", async (req, res) => {
  try {
    ...
    log31.error({ err: error }, "Error importing surat keluar:");
    res.status(500).json({ error: "Failed to import", message: error.message });
  }
});
var google_drive_import_routes_default = router36;

// src/routes/client-upload.routes.ts
import { Router as Router36 } from "express";
import { handleUpload } from "@vercel/blob/client";
var log32 = createLogger("ClientUploadRoutes");
var router37 = Router36();
router37.use(uploadLimiter);
  ...
  try {
    ...
    const webRequest = new globalThis.Request(url, {
      ...
      headers: Object.fromEntries(
        Object.entries(req.headers).filter(([, v]) => typeof v === "string").map(([k, v]) => [k, v])
      ),
    ...
    log32.error({ err: error }, "Client upload handler error");
    res.status(400).json({
      error: error.message || "Upload failed"
    });
  }
});
var client_upload_routes_default = router37;

// src/app.ts
var helmet = helmetModule.default || helmetModule;
var app = express2();
...
app.get("/api/blob-test", authMiddleware, async (req, res) => {
  try {
...
app.use("/api/client-upload", client_upload_routes_default);
if (env.NODE_ENV === "development") {
