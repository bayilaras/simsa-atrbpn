import {
  arsipService
} from "./chunk-MDWP4IF7.js";
import {
  arsipVitalService
} from "./chunk-Y75PW3VJ.js";
import {
  arsipTerjagaService
} from "./chunk-DL6EWBUY.js";
import {
  db,
  env
...
var log5 = createLogger("GoogleDriveService");
function formatPrivateKey(key) {
  if (!key) return "";
  let cleaned = key.replace(/^["']|["']$/g, "");
  cleaned = cleaned.replace(/\\n/g, "\n");
  if (cleaned.includes("\n") && cleaned.includes("-----BEGIN")) {
    log5.info("Private key: restored via \\n replacement");
    return cleaned;
  }
  ...
  if (pemMatch) {
    ...
    log5.info({ headerFound: true, bodyLength: body.length, lineCount: bodyLines.length }, "Private key: reconstructed PEM from flattened key");
    return reconstructed;
  }
  log5.warn("Private key: could not detect PEM structure, using as-is");
  return cleaned;
}
var privateKey = formatPrivateKey(env.GOOGLE_PRIVATE_KEY);
log5.info({
...
var GoogleDriveService = class {
  ...
  async uploadFile(options2) {
    ...
    stream.push(buffer);
    stream.push(null);
    log5.info({ fileName, mimeType, bufferSize: buffer.length, targetFolderId }, "Uploading file to Google Drive");
    ...
    log5.info({ fileId: response.data.id, fileName }, "File created in Drive, setting permissions");
    try {
      ...
      log5.warn({ err: permError?.message }, "Could not set file permissions (may be inherited from Shared Drive)");
    }
    return {
      id: response.data.id,
  ...
  async getFile(fileId) {
    try {
      ...
      log5.error({ err: error }, "Failed to get file:");
      return null;
    }
  }
  ...
  async deleteFile(fileId) {
    try {
      ...
      log5.error({ err: error }, "Failed to delete file:");
      return false;
    }
  }
  ...
  async downloadFile(fileId) {
    try {
      ...
      log5.error({ err: error, fileId }, "Failed to download file from Drive");
      return null;
    }
  }
    ...
    try {
      ...
      if (file && file.buffer) {
        try {
          ...
          log6.info({ driveFileId: driveFile.id, fileName: file.originalname }, "File uploaded to Google Drive");
        } catch (driveError) {
          log6.error({ err: driveError }, "Failed to upload file to Google Drive");
    ...
    try {
      ...
      if (file && file.buffer) {
        try {
          ...
          log6.info({ driveFileId: driveFile.id, fileName: file.originalname }, "File uploaded to Google Drive (update)");
        } catch (driveError) {
          log6.error({ err: driveError }, "Failed to upload file to Google Drive");
          return res.status(500).json({ success: false, error: "Gagal mengunggah file ke Google Drive", message: driveError?.message || "Unknown error" });
        }
...
router.post("/:id/archive-full", canWriteMiddleware(), async (req, res, next) => {
  try {
    const id = req.params.id;
    const { arsipService: arsipService2 } = await import("./arsip.service-DK5ABKI5.js");
    ...
    try {
      ...
      if (file && file.buffer) {
        try {
          ...
          filePath = `gdrive:${driveFile.id}`;
          fileOriginalName = file.originalname;
          log7.info({ driveFileId: driveFile.id, fileName: file.originalname }, "File uploaded to Google Drive");
        } catch (driveError) {
          log7.error({ err: driveError }, "Failed to upload file to Google Drive");
          return res.status(500).json({ success: false, error: "Gagal mengunggah file ke Google Drive", message: driveError?.message || "Unknown error" });
        }
      }
    ...
    try {
      ...
      if (file && file.buffer) {
        try {
          ...
          updateData.filePath = `gdrive:${driveFile.id}`;
          updateData.fileOriginalName = file.originalname;
          log7.info({ driveFileId: driveFile.id, fileName: file.originalname }, "File uploaded to Google Drive (update)");
        } catch (driveError) {
          log7.error({ err: driveError }, "Failed to upload file to Google Drive");
          return res.status(500).json({ success: false, error: "Gagal mengunggah file ke Google Drive", message: driveError?.message || "Unknown error" });
        }
...
var PrintTemplateService = class {
  ...
  async generateDaftarArsipVital(unitKerjaId) {
    const { data } = await import("./arsip-vital.service-YOKROXY4.js").then((m) => m.arsipVitalService.findAll({ unitKerjaId, limit: 1e3 }));
    const doc = new PDFDocument2({ size: "A4", layout: "landscape", margin: this.MARGIN.left });
    const buffers = [];
    doc.on("data", (chunk) => buffers.push(chunk));
  ...
  async generateDaftarArsipTerjaga(unitKerjaId) {
    const { data } = await import("./arsip-terjaga.service-UM5PTQWI.js").then((m) => m.arsipTerjagaService.findAll({ unitKerjaId, limit: 1e3 }));
    const doc = new PDFDocument2({ size: "A4", layout: "landscape", margin: this.MARGIN.left });
    const buffers = [];
    doc.on("data", (chunk) => buffers.push(chunk));
