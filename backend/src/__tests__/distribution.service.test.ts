...
describe('DistributionService', () => {
    ...
    describe('receive', () => {
        it('should mark distribution as received', async () => {
            ...
            const res = await svc.receive('dist-1', 'user-1');
            expect(res.status).toBe('received');
        });

        it('should throw if distribution not found', async () => {
            enqueue([]);
            await expect(svc.receive('missing', 'u1')).rejects.toThrow('Distribution not found');
        });

        it('should throw if already received', async () => {
            enqueue([{ id: 'dist-1', status: 'received' }]);
            await expect(svc.receive('dist-1', 'u1')).rejects.toThrow();
        });
    });
    ...
    describe('reject', () => {
        it('should reject distribution with reason', async () => {
            ...
            const res = await svc.reject('dist-1', 'Salah unit');
            expect(res.status).toBe('rejected');
        });

        it('should throw if distribution not found', async () => {
            enqueue([]);
            await expect(svc.reject('missing', 'reason')).rejects.toThrow();
        });

        it('should throw if already processed', async () => {
            enqueue([{ id: 'dist-1', status: 'processed' }]);
            await expect(svc.reject('dist-1', 'reason')).rejects.toThrow();
        });
    });
    ...
    describe('findById', () => {
        it('should return distribution with surat details', async () => {
            ...
            const res = await svc.findById('dist-1');
    ...
    describe('isDistributed', () => {
        it('should return true when surat has distributions', async () => {
            enqueue([{ count: 3 }]);
            expect(await svc.isDistributed('sm-1')).toBe(true);
        });

        it('should return false when no distributions exist', async () => {
            enqueue([{ count: 0 }]);
            expect(await svc.isDistributed('sm-99')).toBe(false);
        });
    });
});
