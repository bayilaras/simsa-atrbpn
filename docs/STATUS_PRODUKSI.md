...
| Bagian | Hasil |
| --- | --- |
| Vercel Production | Frontend `AexUrmK7MJuQfKvoznVxJRBJ4pNc` (sumber `11959d1`) dan backend `FHaGB81bxWJwYUJnR9FWwgrx7WSo` (sumber `087c8fe`) aktif pada domain utama. Pemeriksaan terakhir 12 September 2026 pukul 10.42 WIB: health dan readiness HTTP 200; database, Blob privat, scanner dan worker dinyatakan siap. Manifest frontend HTTP 200, mode `full`, API same-origin. |
