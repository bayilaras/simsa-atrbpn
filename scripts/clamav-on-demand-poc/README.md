...
Paket diekstrak ke direktori sementara tanpa instalasi sistem. Freshclam mengambil `main`, `daily`, dan `bytecode` dari layanan resmi, dengan `TestDatabases yes` dan `FIPSCryptoHashLimits yes`. Sesuai [source Freshclam1.5.4](https://github.com/Cisco-Talos/clamav/blob/clamav-1.5.4/libfreshclam/libfreshclam_internal.c#L2257), nama detached signature adalah **`nama-versi.cvd.sign`**, misalnya `main-64.cvd.sign`; versi harus cocok dengan header CVD, bukan sekadar menemukan file `.sign` apa saja. Ketiga signature wajib tersedia; `sigtool --info --fips-limits` harus berhasil dengan versi yang cocok dan `Verification OK.`. Tidak ada fallback tanpa tanda tangan. Tidak memakai `curl` untuk menghindari pembatas CDN atau mirror pihak ketiga.

Hanya executable, shared libraries, sertifikat CVD, ketiga CVD beserta signature versi yang cocok dimasukkan ke `vendor/` yang diabaikan Git. Manifest mencatat versi, hash/ukuran CVD dan signature, serta saat verifikasi; runtime memeriksa kedua hash sebelum scan. Runtime menolak build lebih tua dari 24 jam; percobaan perlu build baru sesudah itu. Ini belum merupakan mekanisme pembaruan basis tanda tangan produksi. Build menampilkan daftar nama database, hash, dan keluaran Freshclam yang dibatasi untuk diagnosis; tidak membaca konfigurasi atau rahasia aplikasi.
...
# Mode adaptor native (Preview terisolasi)

Mode bawaan tetap memakai tiga fixture lama. Untuk menguji adaptor backend yang sebenarnya, tambahkan `SIMSA_CLAMAV_NATIVE_POC_ENABLED=1` khusus Preview, dengan `SIMSA_CLAMAV_POC_ENABLED=1` dan Vercel Authentication tetap aktif. Project harus menyertakan berkas di luar Root Directory agar empat sumber backend dapat dibaca saat build. Jangan berikan kredensial aplikasi, database, Blob, atau dokumen pengguna kepada project ini.

`node build.mjs` akan membangun bundle resmi dengan `freshclam`, `sigtool`, sertifikat, dan definisi terverifikasi; Node 24 mentransformasikan empat modul backend ke direktori `native/` tanpa mengimpor aplikasi. Bundle `vendor/` dan `native/` harus masuk function. Direktori keduanya diabaikan Git. Uji lokal tanpa download: `node --test native-build.test.mjs poc.test.mjs`.

Form kosong yang sama menjalankan pemeriksaan kesehatan, refresh resmi dari salinan manifest yang sengaja kedaluwarsa, PDF sintetis 1 KiB dan 10 MiB, pola EICAR, serta pembatalan satu pengulangan PDF kecil. Hanya manifest/definisi yang disalin ke `/tmp`; binary dan sertifikat tetap berasal dari bundle. Deadline seluruh handler 275 detik, setiap operasi paling lama 200 detik. Laporan menyertakan tahap, exit code, waktu, puncak RSS pemanggil+supervisor+keturunan native, hasil refresh, dan bukti pembersihan input setelah pembatalan. Ini belum menguji integrasi antrean/database/Blob produksi.
